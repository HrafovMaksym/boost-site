...
export function ServiceCard({ service, isComingSoon }: ServiceCardProps) {
  ...
  return (
    <div className="group relative h-full">
      ...
      <Card
        className={`relative h-full flex flex-col p-6 md:p-8 bg-[#0b1021]/80 backdrop-blur-xl border-border rounded-3xl transition-all duration-500 shadow-lg overflow-hidden ${
          isComingSoon
            ? "hover:border-accent-primary/30"
            : "hover:border-accent-primary/40 group-hover:-translate-y-1 group-hover:shadow-[0_8px_40px_-12px_rgba(124,58,237,0.3)]"
        }`}
      >
        ...
        <div
          className={`flex-1 ${isComingSoon ? "transition-all duration-500 group-hover:opacity-0 group-hover:-translate-y-4" : ""}`}
        >
          <div className="flex items-center gap-4 mb-6 relative">
            <div
              className={`w-14 h-14 rounded-2xl bg-accent-primary/10 border border-accent-primary/20 flex items-center justify-center relative overflow-hidden transition-transform duration-500 ${
                isComingSoon ? "brightness-75" : "group-hover:scale-110"
              }`}
            >
          ...
          <p
            className={`text-sm leading-relaxed mb-6 ${isComingSoon ? "text-text-muted" : "text-text-secondary"}`}
          >
            {service.description}
          </p>
            ...
            {service.features.map((feature, i) => (
              ...
              >
                <div className="mt-0.5 w-5 h-5 rounded-full bg-accent-primary/10 flex items-center justify-center shrink-0">
                  <Check
                    size={12}
                    className={isComingSoon ? "text-text-muted" : "text-accent-primary"}
                    strokeWidth={3}
                  />
                </div>
        ...
        {isComingSoon ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500">
            <MessageCircle size={32} className="text-accent-primary mb-4" />
            <p className="text-text-primary font-medium mb-6 leading-relaxed">
              Interested in {service.title}? Contact us to discuss your needs!
            </p>
            <Link
              href="/contacts"
              className="px-6 py-2.5 rounded-full bg-accent-primary hover:bg-accent-primary-hover text-white text-sm font-bold transition-all hover:scale-105"
            >
              Contact Us
            </Link>
          </div>
